...
function decodeWithFallback(buf){
  // tenta utf-8; se "Ã" ou "�" aparecer em excesso, tenta windows-1252
  let text = new TextDecoder('utf-8', {fatal:false}).decode(buf);
  const bad = (text.match(/Ã.|�/g)||[]).length;
  if(bad > 5){ // heurística simples
    try {
...
function sliceFromHeader(text){
  ...
  let idx = 0;
  if(findSep(lines[0])) idx = 1;
  // Busca a primeira linha que contenha um dos cabeçalhos base
  const headerIdx = lines.findIndex((ln,i)=> i>=idx && /Protagonistas|Matr[ií]cula|Aluno|Nome/i.test(ln));
  if(headerIdx > idx) return lines.slice(headerIdx).join('\n');
...
async function loadCSV(path){
  ...
  // Delimitador: usa "sep=" se existir na primeira útil; caso contrário PapaParse detecta
  const first = text.split(/\r?\n/)[0] || '';
  const delimiter = findSep(first);
  const parsed = Papa.parse(text, {header:true, skipEmptyLines:true, delimiter: delimiter || undefined});
  return {rows: parsed.data, headers: parsed.meta.fields || Object.keys(parsed.data[0]||{}), rawFirstLine:first};
}
...
function populaFiltros(dados, headerMap){
  const turmaKey = resolveAny(headerMap, ['S/T','ST','Serie/Turma','Série/Turma']);
...
function mostrarDetalhe(row){
  ...
  const turmaKey = resolveAny(hm, ['S/T','ST','Serie/Turma','Série/Turma']);
...
async function main(){
  ...
  // Debug leve para ajudar: mostra primeira linha processada no console
  console.info('Primeira linha utilizada como cabeçalho:', rawFirstLine);
  ...
  const turmaKey = resolveAny(window.headerMap, ['S/T','ST','Serie/Turma','Série/Turma']);
